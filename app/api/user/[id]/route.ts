...
export async function PUT(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    console.log("Form data:", { name, designation, deleteAvatar, hasFile: !!file, currentAvatar: user.avatar });
    ...
    if (deleteAvatar && user.avatar?.public_id) {
      console.log("Attempting to delete avatar with public_id:", user.avatar.public_id);
      ...
      if (deleteResult.errMessage) {
        ...
        return NextResponse.json({ error: `Failed to delete avatar: ${deleteResult.errMessage}` }, { status: 500 });
      }
      user.avatar = null;
    ...
    if (file) {
      if (user.avatar?.public_id) {
        console.log("Replacing avatar, deleting old public_id:", user.avatar.public_id);
        const deleteResult = await deletePhoto(user.avatar.public_id);
      ...
      if (uploadResult.errMessage) {
        ...
        return NextResponse.json({ error: `Failed to upload avatar: ${uploadResult.errMessage}` }, { status: 500 });
      }
